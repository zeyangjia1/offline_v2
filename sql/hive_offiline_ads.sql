...
create
external table ads_Three_day_proportion(
...
create
external table ads_Brand_re_purchase_rate(
...
WITH recent_purchases AS (
    SELECT tm_id,
           ...
           dt,
           recent_days
    FROM dwt_Grand_summary_1 LATERAL
    VIEW
    explode(array(1
   , 7
   , 30)) days AS recent_days
WHERE
    dt>= date_add('2025-03-25'
    , -recent_days+1)
    )
...
insert
into table ads_Brand_re_purchase_rate
SELECT tm_id,
       recent_days,
       "2025-03-25",
       concat(cast(ROUND(repeat_users / total_users * 100, 2) as string), '%') AS lv
FROM total_and_repeat_users;

select *
from ads_Brand_re_purchase_rate;

--最近七日连续三天下单的用户数占比
insert into table ads_Three_day_proportion
select concat((c.user_nums / d.counts) * 100, '%') as lv,
       c.miaoshu
from (select count(distinct b.user_id)  as user_nums,
             "最近七日连续三天下单占比" as miaoshu
      from (select a.dt      as dt,
                   a.user_id as user_id,
                   lag(dt, 2)   over (partition by user_id order by dt) as days
            from (select distinct user_id, dt from dwt_Grand_summary_1) a
            where a.dt >= date_sub('2025-03-25', 7)) b
      where datediff(b.dt, b.days) = 2) c
         join
     (select count(*) as counts
      from dwt_Grand_summary_1
      where dt >= date_sub('2025-03-25', 7)) d;



insert into table ads_page_path
select '2025-03-25'              as dt,
       recent_days,
...
from (select *,
             page_id as    soure,
             lead(page_id) over (partition by sid order by ts) as targent, row_number() over (partition by sid order by ts) as step
      from (select *,
                   concat(mid_id, '-', last_value(`if`(last_page_id is null, ts, null),
                                                  true) over (partition by mid_id order by ts)) sid
            from dwd_page_log lateral view explode (Array(1,7,30)) tmp as recent_days
            where dt>=date_add('2025-03-25', -recent_days+1)) t1) t2
